...
def main():
    ...
    if microscope == 'cv7k':
        ...
        channel_names, n_sites, n_slices, n_tp, wells = extract_metadata_cv7k(img_files)
    ...
    if output_path.joinpath(f'Assayplate.zarr').exists():
        z = zarr.open(output_path.joinpath(f'Assayplate.zarr'), mode='a')
        print('opening in append mode')
    else:
        z = zarr.open(output_path.joinpath(f'Assayplate.zarr'), mode='w')
        print('making new file')
    for well in wells:
        for site in range(1, n_sites+1):
            for channel in channel_names:
                for level in range(0, N_LEVELS):
                    if not hasattr(z, f'{well}/{site}/intensity_images/{channel}/level_{level}'):
                        d = z.create_dataset(
                            f'{well}/{site}/intensity_images/{channel}/level_{level:02d}',
                            shape=[n_tp, n_slices,
                                   img_dims[0]/2**level,
                                   img_dims[1]/2**level],
                            chunks=chunk,
