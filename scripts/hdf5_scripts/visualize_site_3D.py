...
def visualize_site(settings, site, intensities=True, labels=True, boundaries=True, tracks=True, blobs=True):
    ...
    if blobs:
        ...
        for blob_file in blob_files:
            ...
            viewer.add_points(blobs[['timepoint', 'centroid-0', 'centroid-1', 'centroid-2']], name=str(blob_file).replace('.csv', '').split('site_%04d_' % site)[1],
                              face_color='transparent',
                              edge_color='white',
                              size=blobs['size'],
                              visible=False,
                              scale=(1, settings['scaling'], 1, 1))
...
visualize_site(settings, 1, boundaries=False, blobs=True)